...
// </VimperatorPlugin>`;
// //}}}
dactyl.plugins.nextlink = (function() {

// initialize //{{{
if (!dactyl.plugins.libly) {
  dactyl.log("nextlink: needs _libly.js");
  return;
}
...
var prevMap = dactyl.globalVariables.nextlink_prevmap || DEFAULT_PREVMAP;
var nextMap = dactyl.globalVariables.nextlink_nextmap || DEFAULT_NEXTMAP;

var isFollowLink = typeof dactyl.globalVariables.nextlink_followlink == "undefined" ?
                   false : $U.eval(dactyl.globalVariables.nextlink_followlink);
...
const nositeinfoActions = {
  ...
  e: function(doc, count) {
       ...
       return dactyl.echo("No SITEINFO match " + url);
     },
  n: function() true,
};
var actpattern = dactyl.globalVariables.nextlink_nositeinfo_act || "e";
...
NextLink.prototype = {
  ...
  nextLink: function(count) {
    if (!this.initialized) {
      dactyl.echo("before initialized.");
      return false;
    }
  ...
  customizeMap: function(context) {
    mappings.add(context.browserModes, [ prevMap ], "customize by nextlink.js",
      function(count) context.nextLink(count > 0 ? -1 * count : -1),
      { count: true });

    mappings.add(context.browserModes, [ nextMap ], "customize by nextlink.js",

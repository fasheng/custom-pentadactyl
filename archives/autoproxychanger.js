...
dactyl.plugins.AutoProxyChanger = (function() {
var gVar = options;
...
function init() {
  ...
  if (acmanager.length > 0) {
    autocommands.add("LocationChange", '.*', 'js dactyl.plugins.AutoProxyChanger.autoApplyProxy()');
...
function applyProxyByName( name ) {
  if (!name) {
    dactyl.echo( dump(restore())+'usage:proxy [setting name]' );
    return;
  }
...
group.commands.add(['proxy'], 'Proxy settings',
  function(args) {
...
group.commands.add(['toggleautoproxy','aprxy'], 'Toggle auto proxy changer on/off',
  function() {
    manager.isEnable = !manager.isEnable
    dactyl.echo('autoproxy:'+ manager.isEnable ? 'ON' : 'OFF');
  }, {}
);

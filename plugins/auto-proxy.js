...
group.commands.add(["autoproxy", "ap"], "Change autoproxy status",
    function (args) {
        if (args.length == 0) { // print current autoproxy status
            ...
            //	mode = 'auto';f
            //}
            dactyl.echomsg("proxyMode is " + mode);
        } else {
            ...
            if (arg.indexOf('d') == 0) {
                ...
                dactyl.echomsg("set proxyMode to " + mode);
                ...
                dactyl.echoerr("mode should be one of 'disabled', 'auto' or 'global'");
            }
        }
    }, {
        argCount: "?",
...
group.commands.add(["switchproxy", "sp"], "Switch autoproxy status",
    function (args) {
        if (args.length == 0) {
            var mode = getPreferenceValue("extensions.autoproxy.", "proxyMode");
            //if (mode == null)
            if (mode == 'auto') {
                ...
                dactyl.echomsg("proxyMode is " + mode + ", switch proxyMode to " + newmode);
            }
            else if (mode == 'global') {
                newmode = 'auto';
                setPreferenceValue("extensions.autoproxy.", "proxyMode", newmode);
                ...
                dactyl.echoerr("proxyMode is not 'auto' or 'global'");
            }
        }
        else {
            dactyl.echoerr("We dont need args...");
        }
    }, {
        argCount: "?",
    }
...
group.commands.add(["switchdefaultproxy", "sdp"], "Switch autoproxy default proxy",
    function (args) {
        if (args.length == 0) {
            var default_proxy = getIntPreferenceValue("extensions.autoproxy.", "default_proxy");
            dactyl.echomsg("default proxy is " + default_proxy);
        } else {
            if (isNaN(args)) {
                dactyl.echoerr("default proxy should be number");
            }
            else {
                setIntPreferenceValue("extensions.autoproxy.", "default_proxy", args);
                dactyl.echomsg("switch autoproxy default proxy to " + args);
            }
        }
    }, {
        argCount: "?",
...
group.commands.add(["switchproxyrefresh", "spr"], "Switch autoproxy status and refresh",
    function (args) {
        if (args.length == 0) {
            var mode = getPreferenceValue("extensions.autoproxy.", "proxyMode");
            //if (mode == null)
...
group.commands.add(["foxyproxy","fp"],"set FoxyProxy Mode",
	function (args) {
	if (args.length == 0){
		var proxyMode;

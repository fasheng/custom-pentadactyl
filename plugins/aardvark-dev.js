...
var Aardvark = Class("Aardvark", {
    ...
    closePanels: function closePanels() {
        let panels = this.container.children
                         .filter(function (elem) DOM(elem).highlight.has("AardvarkDBox"));
        panels.remove();
    ...
    select: function select(elem) {
        let panels = this.container.children
                         .filter(function (elem) elem.dbox);
        if (panels.length)
    ...
    colorize: function colorize(elem) {
        function val() Math.round(Math.random() * 16).toString(16);
        this.undoStack.push({
            styles: {
...
//-------------------------------------------------
    ...
    choose: function choose(elem) {
        ...
        if (elem.isAardvark || DOM(elem).ancestors.some(function (e) e.isAardvark))
            return;
